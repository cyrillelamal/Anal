...
def task2():
    ...
    fig.add_trace(go.Scatter(
        x=x, y=y,
        mode='lines',
        name='Распределение сельскохозяйственных предприятий',
    ))
    fig.update_layout(
        title='Распределение сельскохозяйственных предприятий по числу производственных подразделений (Полигон частот)',
        xaxis_title='Число производственных предприятий',
        yaxis_title='Частоты'
    )
    fig.show()
